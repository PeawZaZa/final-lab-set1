...
### Log Service
เขียน service ใหม่ทั้งหมด (ไม่มีใน Week 12) รับ log จาก services อื่นผ่าน `POST /api/logs/internal` โดยไม่ต้องมี JWT เพราะเรียกภายใน Docker network เท่านั้น ส่วน `GET /api/logs/` และ `GET /api/logs/stats` ต้องมี JWT และ role=admin เท่านั้นถึงจะเข้าได้

### Frontend
ปรับ `index.html` จาก Week 12 โดยลบ Register tab, ลบ Users nav, เปลี่ยน `apiFetch()` → `fetch()` ธรรมดา, เปลี่ยน `user.name` → `user.username`, เปลี่ยน endpoint `/api/users/me` → `/api/auth/me`

สร้าง `logs.html` ใหม่ทั้งหมด (Week 12 ใช้ localStorage แต่ Set 1 ดึงจาก Log Service DB) มี filter by service/level, client-side search, auto-refresh ทุก 5 วินาที
